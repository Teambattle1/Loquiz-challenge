...
const Showtime = ({ photos, onClose, onShowtimeComplete }: ShowtimeProps) => {
    ...
    const [isShowtimeMode, setIsShowtimeMode] = useState(false); // Full SHOWTIME flow
    const music = useMusicPlayer();
    const showtimeCompleteTriggered = useRef(false);

    // The photos used in slideshow — selected subset or all
        ...
        if (view === 'slideshow' && isPlaying && slideshowPhotos.length > 0) {
            interval = setInterval(() => {
                setCurrentIndex(prev => {
                    const next = prev + 1;
                    // In SHOWTIME mode: stop after last photo and transition to results
    ...
    }, [isPlaying, view, slideshowPhotos.length, isShowtimeMode, music, onShowtimeComplete]);
    ...
    }, [view, slideshowPhotos.length, onClose]);
    ...
    // Full SHOWTIME flow: slideshow + music → results reveal
    const startShowtime = () => {
            ...
            <div className="fixed inset-0 z-50 bg-zinc-950/95 backdrop-blur-xl flex flex-col animate-fade-in">
                ...
                <div className="p-4 md:p-6 flex justify-between items-center bg-black/40 border-b border-white/5 shrink-0">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        {/* SHOWTIME button — full flow with music + results reveal */}
                        <button
                            onClick={startShowtime}
                            className="px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-wider bg-gradient-to-r from-pink-600 to-orange-600 text-white border border-pink-500/50 hover:from-pink-500 hover:to-orange-500 transition-all shadow-[0_0_20px_rgba(236,72,153,0.3)] hover:shadow-[0_0_30px_rgba(236,72,153,0.5)]"
                        >
                            ★ Showtime
                        </button>
                        {/* Play selected slideshow (no music) */}
                        {selectedIds.size > 0 && !selectMode && (
                            <button
                                onClick={startSelectedSlideshow}
                                className="px-3 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-wider bg-orange-600 text-white border border-orange-500 hover:bg-orange-500 transition-all shadow-[0_0_15px_rgba(234,88,12,0.3)]"
                            >
                        ...
                        {!selectMode && selectedIds.size === 0 && (
                            <button
                                onClick={() => enterSlideshow(0)}
                                className="px-3 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-wider bg-orange-600 text-white border border-orange-500 hover:bg-orange-500 transition-all"
                            >
                                Play All ▶
                            </button>
                        )}
                ...
                {selectMode && (
                    <div className="flex items-center gap-3 px-4 md:px-6 py-2 bg-zinc-900/80 border-b border-zinc-800/50 shrink-0">
                        <span className="text-zinc-500 text-xs uppercase tracking-wider font-bold">Select photos for slideshow:</span>
            ...
            <div className="absolute top-0 left-0 w-full p-4 flex justify-between items-center z-20 bg-gradient-to-b from-black/80 to-transparent">
                <div className="flex items-center gap-4">
                    ...
                    <span className="text-xs font-mono text-zinc-400 bg-black/50 px-2 py-1 rounded border border-zinc-700">
                        ...
                        {selectedIds.size > 0 && <span className="text-orange-400 ml-1">(curated)</span>}
                    </span>
                </div>
